...
def reddit(times_voted, link, anti_key, site_key, chr_path, inspect_id):
    api_key = anti_key

    num_google_bots = file_size("users.txt")
    num_reddit_bots = file_size("reddit_users.txt")
    use_bot = int(
        input("to perform this vote manipulation would you like to use\n1.Your reddit bots\nor\n2.Your google bots"))

    # if you decide to use your reddit bots
    if (use_bot == 1):

        # if you dont have enough reddit bots, make some more now and use later
        if (times_voted > num_reddit_bots):
            ...
            # if you want to make your reddit bots using a proxy(paid or free)
            ...
            # if you want to make your reddit bots without using a proxy
            else:
                df = open('reddit_users.txt', 'a')
                x = 0
                num_needed = times_voted - num_reddit_bots
        ...
        # if you do have enough reddit bots to upvote/downvote
        else:
            ip_type = int(input("would you like to\n1.Use a pre paid proxy\n2.Use a free generated one(less "
                                "successful)\n3.Use no proxy"))

            # if you want to use a proxy(paid or free) to help your reddit bots
            if (ip_type == 1 or ip_type == 2):
                proxy_ip_port = ''
                proxy_list = []
                if (ip_type == 1):
                ...
                while (x < times_voted):
                    ...
                    browser = webdriver.Chrome(executable_path=path, desired_capabilities=capabilities)
                    browser.implicitly_wait(5)
                    browser.get('https://www.reddit.com/account/login/')
                    sleep(5)

                    username_input = browser.find_element_by_css_selector("input[name='username']")
                    password_input = browser.find_element_by_css_selector("input[name='password']")
                    log_in = browser.find_element_by_xpath('//button[@type="submit" and text()="Log In"]')
                    ...
                    # vote button find and click

                    id = '//button[@id = "{}"]'.format(inspect_id)
                    vote_button = browser.find_element_by_xpath(id)
                    vote_button.click()

                    x += 1

            # if you want to upvote/downvote with your reddit bots without using a proxy
            else:
                df = open('reddit_users.txt', 'r')
                x = 0
                content = df.readlines()
    ...
    # if you decide to use your google bots
    elif (use_bot == 2):

        # if you dont have enough bots, make some more now and use later
        ...
        # if you do have enough google bots
        else:
            df = open('reddit_users.txt', 'r')
            x = 0
            content = df.readlines()
            while (x < times_voted):
                ...
                google_login = browser.find_element(By.XPATH, "//form[@id='google-sso']")
                google_login.click()

                email_input = browser.find_element_by_css_selector("input[name='identifier']")
                email = line[0]
                email_input.send_keys(email)

                next_button = browser.find_element(By.XPATH, "//input[[@type='button']")
                next_button.click()
                ...
                login_button = browser.find_element(By.XPATH, "//input[[@type='button']")
                login_button.click()
...
def make_account(number_of_accounts, anti_key, chr_path, sms_uid, sms_pass):
    ip_type = int(input("would you like to\n1.Use a pre paid proxy\n2.Use a free generated one(less "
                        "successful)\n3.Use no proxy"))
    # if you want to use a proxy(paid or free) to make your google bots
    if (ip_type == 1 or ip_type == 2):
        proxy_ip_port = ""
        proxy_list = []
    ...
    # if you want to make your google bots without using a proxy
    else:
        df = open('users.txt', 'a')
        x = 0
...
def main():
    ...
    if (x == 1):
        option = int(input("would you like to check how many\n1.Reddit bots you have\nor\n2.Google bots you have"))

        if (option == 1):
            num_bots = file_size('reddit_users.txt')
            print("you have {} reddit bots").format(num_bots)

        elif (option == 2):
            num_bots = file_size('users.txt')
            print("you have {} Google bots").format(num_bots)
        ...
        button_id = input("Please enter the div inspect element id of the button: ")
        reddit(num_downvotes, post_link, anticaptcha_key, site_key, chr_path, button_id)
